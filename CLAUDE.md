...
- **`schema.py`** — canonical Pydantic v2 models with `extra="forbid"`. `SCHEMA_VERSION = 2` is the wire-format version between renderer and orchestrator; **bump it when changing the wire shape** (last bumped in v0.2 by the cross-DAG sensor feature, which added `RenderedTask.external_ref` and `DiffDocument.sensor_mismatches`). `RenderedDag`, `ExternalTaskRef`, and `SensorMismatch` all have model validators enforcing cross-field invariants — follow that pattern when adding new shapes.
...
- **`validators/cross_dag.py`** — called by the orchestrator after `compute_diff()` to detect PR-introduced `ExternalTaskSensor` mismatches (missing/incorrect `execution_delta`, dangling targets). Uses `croniter` for cron arithmetic and operates only on canonical schema types — no Airflow imports. New cross-DAG checks should land here, not in `diff.py`.
